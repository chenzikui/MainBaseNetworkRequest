...
@class DHBBaseNetworkRequest;
@class AFHTTPRequestOperation;

static NSString * const DHBRequestAuthorizationErrorMessage = @"DHBRequestAuthorizationErrorMessage";

typedef void (^RLBaseNetworkRequestSuccessBlock)(DHBBaseNetworkRequest *DAO,id data);
typedef void (^RLBaseNetworkRequestFailedBlock)(DHBBaseNetworkRequest *DAO,NSError *error);

@interface DHBBaseNetworkRequest : NSObject
{
...
- (void)postJSONDataSuccess:(RLBaseNetworkRequestSuccessBlock)success
                    failure:(RLBaseNetworkRequestFailedBlock)failure;
