...
//    NSLog_Red(@"%@",[self parameters_get]);
    ...
    operation = [defaultManager GET:[self fullURLPath] parameters:[self parameters_get] success:^ void(AFHTTPRequestOperation *operation, id responseObject) {
...
- (id)parameters
{
    ...
    [val setValue:ServerVer forKey:@"version"];
//    NSLog_Red(@"skey = %@",val);
    ...
    if (![returnData isKindOfClass:[NSDictionary class]])
    {
        ...
        if(code == 119 || code == 200){
            ...
            [[DHBAlertView sharedManagered]  setOnButtonTouchUpInside:^(CustomIOSAlertView *alertView, int buttonIndex) {
                ...
                [userDefatuls removeObjectForKey: @"tiyan"];
        ...
        else if(self.isReturnDataValidate &&(code == 101 || code == 102 || code == 110 ||code == 401)){
